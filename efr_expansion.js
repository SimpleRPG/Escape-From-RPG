...
function firearmAttack(){
 ...
 if(w.kind==="firearm"){if((w.durability||0)<=0){window.logMessage?.("武器が壊れています");return;}if((w.ammo||0)<=0){window.logMessage?.("弾切れ。Rでリロード");return;}w.ammo--;w.durability--;window.attackTimer=w.cooldown||.3;window.attackFlash=.14;let t=null,b=Infinity;for(const e of window.enemies||[]){if(e.dead)continue;const dx=e.x-window.player.x,dy=e.y-window.player.y,d=Math.hypot(dx,dy)||1;if(d>w.range+(e.r||15))continue;if(!window.playerCanSeeEnemy(e))continue;const targetAngle=Math.atan2(dy,dx),aimAngle=Math.atan2(window.player.facingY,window.player.facingX);let delta=targetAngle-aimAngle;while(delta>Math.PI)delta-=Math.PI*2;while(delta<-Math.PI)delta+=Math.PI*2;delta=Math.abs(delta);if(delta>0.22)continue;const score=d+delta*180;if(score<b){b=score;t=e;}}if(t){t.hp-=w.damage;if(t.hp<=0){t.dead=true;t.loot=loot("敵");}}return;}
